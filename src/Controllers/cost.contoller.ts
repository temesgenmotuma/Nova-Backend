  ...
  try {
    ...
    res.status((error as any).statusCode || 500).json({
      message:
        ...
        "Failed to create cost. Please try again later.",
    });
  }
};
...
export const getCostById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status((error as any).statusCode || 500).json({
      message:
        (error as Error).message ||
        "Failed to retrieve cost. Please try again later.",
    });
  }
};
...
export const getCosts = async (req: Request, res: Response) => {
  ...
  try {
    const { year, limit = 10, offset = 0, lotId } = data;

    const { count, costs } = await costModel.getCostsByYear(
      year,
      limit,
      offset,
      lotId
    ...
    res.status((error as any).statusCode || 500).json({
      message:
        (error as Error).message ||
        "Failed to retrieve costs. Please try again later.",
    });
  }
};
...
export const deleteCost = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status((error as any).statusCode || 500).json({
      message:
        (error as Error).message ||
        "Failed to delete cost. Please try again later.",
    });
  }
};
